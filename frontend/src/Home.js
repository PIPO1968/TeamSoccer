...
function Home({ user, clubData, onLogout }) {
    ...
    return (
        <div className="main-content-with-sidebar" style={{ display: 'flex', flexDirection: 'column', minHeight: '100vh' }}>
            {/* Fila superior: nombre del club (sidebar) y escudo */}
            <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center', marginTop: 24, marginLeft: 240, gap: 18 }}>
                <div style={{ fontWeight: 'bold', fontSize: '1.2rem', color: '#1a2a44', whiteSpace: 'nowrap' }}>{clubData?.teamName || 'Nombre del club'}</div>
                <img src={clubData?.escudoUrl || escudo} alt="Escudo del club" style={{ width: 80, height: 80, borderRadius: 12, objectFit: 'cover', boxShadow: '0 2px 8px #0002', background: '#eaeaea' }} />
            </div>
            {/* Contenido principal de la página, centrado */}
            <main className="club-main club-main-wide" style={{ margin: '32px auto 0 auto', paddingTop: 24, display: 'flex', flexDirection: 'column', gap: 24, alignItems: 'center', width: '100%', maxWidth: 900 }}>
